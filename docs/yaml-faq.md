...
YAML always parses literal values as strings, _unless_ the value is wholly numeric (including canonical, integer, fixed point, exponential, octal or hex), a timestamp ([ISO 8601](https://en.wikipedia.org/wiki/ISO_8601) format) or a Boolean (`true`, `false`), or if your value starts with a `&` or `*` in which case it will assume [anchor/alias](#aliases-and-anchors). Kubernetes students, this last case applies especially to CronJob schedules.
