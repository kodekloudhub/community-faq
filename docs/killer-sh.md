...
* Both sessions will have the same questions, unless the Kubernetes version of the exam is updated between the time you do session 1 and session 2, in which case _some_ questions _may_ change, depending on what has changed with the Kubernetes upgrade. Any questions relating to features that have been retired (e.g. PodSecurityPolicy in 1.25) are definitely replaced.<br/>Note that some Killer sessions now have a Set A and Set B. These are different sets of questions, but Set A will be the same every time you do it, as will Set B.
