...
`vi` is the most common text editor for Linux systems and is the preferred editor for taking the Linux Foundation hands-on exams, and KodeKloud labs. There are several versions such as `vi`, `vim`, `vim enhanced`, `neovim` etc., but for the purposes of this guide they can all be treated as equal.
