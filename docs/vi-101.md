...
`vi` is the most common text editor for Linux systems and is the preferred editor for taking the Linux Foundation hands-on exams, and KodeKloud labs. There are several versions such as `vi`, `vim`, `vim enchanced`, `neovim` etc., but for the purposes of this guide they can all be treated as equal.
...
* `SHIFT + V` - Enter visual line mode. You can select multiple lines of text and perform operations like `dd`, `y`, `>`, `<` on the selected lines, also `n>` or `n<` where `n` is a number to outdent/indent `n` tab stops.
