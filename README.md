...
**Update April 2026**: Note that there is now a feature that permits you to use a phone for the check-in process. You are given a QR to scan which redirects to a page that connects your phone to the proctor so that you can use it to do the room scan. The phone still cannot be used once the exam itself begins so your webcam must still work. Wait until the proctor is ready to start the room scan before scanning the QR code.
